	...
	<script type="text/javascript">
		$(function(){
			...
			$("input[name=privilegeIds]").click(function(){
				//当选择或取消某个权限是，同时也选中或取消所有的下级权限
				$(this).siblings("ul").find("input").attr('checked',this.checked);
				
				//当选中某个选权限，应同时选中直系上级权限
				if(this.checked==true){//可以直接写为if(this.checked){...}
					$(this).parents("li").children("input[name=privilegeIds]").attr('checked',this.checked);
				}else{
					//当取消某个权限时，如果统计的权限都没有选择，就取消上级权限
						...
						<thead>
							<tr align="LEFT" valign="MIDDLE" id="TableTitle">
								<td width="300px" style="padding-left: 7px;">
									...
									onclick=" $('input[name=privilegeIds]').attr('checked',this.checked) " /> <label for="cbSelectAll">全选</label>
								</td>
							</tr>
						</thead>
							...
							<tr class="TableDetail1">
								...
								<td>
									...
									<s:iterator value="topPrivilegeList">
										<li>
										<input type="checkbox" name="privilegeIds" value="${id}" id="cd_${id}" <s:property value="%{id in privilegeIds?'checked':''}"/> />
